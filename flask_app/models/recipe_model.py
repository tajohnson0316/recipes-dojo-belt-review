...
class Recipe:
    ...
    # SELECT one recipe by ID with a list of users that have favorited it
    @classmethod
    def get_one_with_favorites(cls, data):
        ...
        current_recipe.user = user_model.User.get_one_by_id({"id": current_recipe.user_id})

        for row in results:
            if row["u.id"] is not None:
                favorite_user = {
                    "id": row["u.id"],
                    "first_name": row["first_name"],
                    "last_name": row["last_name"],
    ...
    # DELETE a recipe by ID, first making sure to delete any corresponding favorites
    @classmethod
    def delete_one_by_id(cls, data):
        query = """ 
    ...
    # Method to validate recipe forms
    @staticmethod
